...
(function() {
    ...
    // =====================================================================
    // STORAGE KEY — per wioska
    // =====================================================================
    function getStorageKey() {
        try {
            if (typeof game_data !== 'undefined' && game_data.village && game_data.village.id) {
                return `tribalWarsBuilderConfig_v${game_data.village.id}`;
            }
        } catch(e) {}
        const match = window.location.search.match(/[?&]village=(\d+)/);
        if (match && match[1]) {
            return `tribalWarsBuilderConfig_v${match[1]}`;
        }
        throw new Error('Nie można ustalić ID wioski — skrypt wstrzymany');
    }
    ...
    function handleRewards() {
        if (isConstructionInProgress()) {
            debugLog('Budowa w toku — pomijam odbieranie nagród');
            return false;
        }
 
        if (claimRewardInDialog()) return true;
 
        if (document.querySelector('#new_quest')) {
    ...
    function getWaitTimeForBuilding(buildingName) {
        try {
            ...
            const inactiveSpan = row.querySelector('.build_options .inactive');
            if (!inactiveSpan) return null;
 
            if (inactiveSpan.style.display === 'none') return null;
 
            const text = inactiveSpan.textContent.trim();
            if (!text) return null;
 
            debugLog(`Wait text dla ${buildingName}: "${text}"`);
 
            if (!text.match(/\d{1,2}:\d{2}/)) {
                debugLog(`Blokada bez czasu dla ${buildingName}: "${text}"`);
                return 'SKIP';
            }
            ...
            const isJutro = text.toLowerCase().includes('jutro');
            if (isJutro || target <= now) {
    ...
    function checkAndBuild() {
        ...
        try {
            if (isConstructionInProgress()) {
                debugLog('Budowa w toku — czekam na zakończenie');
            ...
            if (lvl !== null && lvl < item.targetLevel) {
                if (canBuildResource(item.building)) {
                    ...
                    if (waitMs === 'SKIP') {
                        debugLog(`Pomijam krok ${item.building} poz.${item.targetLevel} — blokada bez czasu`);
                        setStatusMessage(`⏭️ Pomijam: ${name} poz.${item.targetLevel} (blokada), przechodzę dalej`, '#a06000');
                        cfg.buildSequence.shift();
                        saveConfig(cfg);
                        setTimeout(() => checkAndBuild(), 500);
                    } else if (waitMs && waitMs > 0) {
    ...
    function createUI() {
        ...
        // Pokaż aktywny klucz wioski w nagłówku
        const villageLabel = STORAGE_KEY.replace('tribalWarsBuilderConfig_', '');
        wrap.innerHTML = `
            <h3 style="margin:0 0 2px 0;font-size:14px;font-weight:bold;">Auto Builder Settings</h3>
            <div style="color:#888;font-size:10px;margin-bottom:4px;">Wioska: <b>${villageLabel}</b> &nbsp;|&nbsp; klucz: <code>${STORAGE_KEY}</code></div>
    ...
    debugLog(`Skrypt WASILEWSKI v1.7.3 uruchomiony. Storage key: ${STORAGE_KEY}`);
